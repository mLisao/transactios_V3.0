package com.jiangguo.transactios.receiver;
				...
				if (fromId != null
					...
					if (TextUtils.isEmpty(tag)) {// 不携带tag标签--此可接收陌生人的消息
						...
						if (tag.equals(BmobConfig.TAG_ADD_CONTACT)) {
								...
								if (toId.equals(currentUser.getObjectId())) {
									if (ehList.size() > 0) {// 有监听的时候，传递下去
										...
										showOtherNotify(context,
												message.getNick(), toId,
												message.getNick() + "请求添加好友",
							...
							String nick = BmobJsonUtil.getString(jo,
									BmobConstant.PUSH_KEY_TARGETNICK);
							...
							// 显示通知
							showOtherNotify(context, nick, toId, nick
